...
namespace Scryber.OpenType.UnitTests
{
    ...
    public static class ValidateNoto
    {
        ...
        public static void AssertInfo(ITypefaceInfo info, string source, int testIndex)
        {
            ...
            {
                Assert.IsNotNull(info.Source, "Noto info Path was null for test " + testIndex);
                Assert.AreEqual(source, info.Source, "Noto info Path was not equal to " + source + " for test " + testIndex);
            }

            Assert.IsNotNull(info.Fonts, "Noto references was null for " + testIndex);
            Assert.AreEqual(1, info.Fonts.Length, "Noto references was not 1 for " + testIndex);
